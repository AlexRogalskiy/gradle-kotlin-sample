...
class Dijkstra(graph:DWGraph, val from:Int) {
     ...
     */
    private val distTo:DoubleArray =
     ...
     */
    private val edgeTo:Array<DWGraph.Edge?>=

    arrayOfNulls(graph.V)
     ...
     *
     * @param v the destination vertex
     * @return `true` if there is a path from the source vertex `s` to vertex `v`; `false` otherwise
     */
    fun hasPathTo(v:Int):
     ...
     *
     * @param v the destination vertex
     * @return a shortest path from the source vertex `s` to vertex `v` as an iterable of edges, and
    ...
    {
        if (!hasPathTo(v)) {
            throw NoSuchPathException("There is no path from [$from] to [$v]")
        }

...
tasks.register("generateCircleciConfig", GenerateCircleciConfig::class) {
  jdks = listOf(8, 11)
  crossVersion = mapOf(
    "gradle51" to CrossVersion("5.1.1", listOf(8, 11)),
    "gradle50" to CrossVersion("5.0", listOf(8, 11)),
    ...
    outputFile.writeText((
      """
        ...
        """
              ...
              wrapper:
            """ + (mapOf(
        "current" to GradleVersion.current().getVersion()
      ) + crossVersion.values.asSequence().map(CrossVersion::gradle)
        .map { it.replace('.', '-') to it }).map { (name, version) ->
        """
